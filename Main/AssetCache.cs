...
namespace AngelPearl.Main
{
	public static class AssetCache
	{
		...
		public static void LoadMap(ContentManager contentManager, GameMap map)
		{
			if (map == GameMap.None || MAPS.ContainsKey(map)) return;
			LdtkJson mapData = LdtkJson.FromJson(contentManager.Load<string>($"Maps/{map.ToString().Replace('_', '/')}"));
			MAPS.Add(map, mapData);
		}

		public static void LoadMusic(ContentManager contentManager, GameMusic music)
		{
			if (music == GameMusic.None || MUSIC.ContainsKey(music)) return;
			Song musicData = contentManager.Load<Song>($"Music/{music.ToString().Replace('_', '/')}");
			MUSIC.Add(music, musicData);
		}

		public static void LoadShader(ContentManager contentManager, GameShader shader)
		{
			if (shader == GameShader.None || SHADERS.ContainsKey(shader)) return;
			Effect shaderData = contentManager.Load<Effect>($"Shaders/{shader.ToString().Replace('_', '/')}");
			SHADERS.Add(shader, shaderData);
		}

		public static void LoadSound(ContentManager contentManager, GameSound sound)
		{
			if (sound == GameSound.None || SOUNDS.ContainsKey(sound)) return;
			SoundEffect soundData = contentManager.Load<SoundEffect>($"Sounds/{sound.ToString().Replace('_', '/')}");
			SOUNDS.Add(sound, soundData);
		}

		public static void LoadSprite(ContentManager contentManager, GameSprite sprite)
		{
			if (sprite == GameSprite.None || SPRITES.ContainsKey(sprite)) return;
			Texture2D spriteData = contentManager.Load<Texture2D>($"Sprites/{sprite.ToString().Replace('_', '/')}");
			SPRITES.Add(sprite, spriteData);
		}

		public static void LoadView(ContentManager contentManager, GameView view)
		{
			if (view == GameView.None || VIEWS.ContainsKey(view)) return;
			string viewData = contentManager.Load<string>($"Views/{view.ToString().Replace('_', '/')}");
			VIEWS.Add(view, viewData);
		}
