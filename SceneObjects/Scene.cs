...
namespace AngelPearl.SceneObjects
{
	...
	public class Scene
	{
		...
		public virtual void Draw(GraphicsDevice graphicsDevice, SpriteBatch spriteBatch, RenderTarget2D gameRender)
		{
			...
			spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.DepthRead, RasterizerState.CullCounterClockwise, shader, null);
			DrawBackground(spriteBatch);
			spriteBatch.End();

			shader = (entityShader == null) ? null : entityShader.Effect;
			spriteBatch.Begin(SpriteSortMode.BackToFront, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.DepthRead, RasterizerState.CullCounterClockwise, shader, cameraMatrix);
			...
			if (OverlayList.Count > 0)
			{
				...
				spriteBatch.Begin(SpriteSortMode.BackToFront, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.DepthRead, RasterizerState.CullCounterClockwise, shader, null);
		...
		public virtual void DrawOverlay(SpriteBatch spriteBatch)
		{
			...
			{
				if (overlay.DrawOnNewLayer)
				{
					spriteBatch.End();
					var shader = (interfaceShader == null) ? null : interfaceShader.Effect;
					spriteBatch.Begin(SpriteSortMode.BackToFront, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.DepthRead, RasterizerState.CullCounterClockwise, shader, null);
				}
