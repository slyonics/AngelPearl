...
namespace AngelPearl.SceneObjects.Widgets
{
	...
	{
		public static readonly Color DEFAULT_TEXT_COLOR = new Color(224, 0, 127);
		...
		private void AdvanceCharacter()
		{
			...
			if (queueElement.text.Length == 0)
			{
				...
				{
					textElement = new TextElement() { line = queueElement.line, color = queueElement.color, offset = queueElement.offset + 16 };
					textLines.Add(textElement);
				}
			}
		...
		private List<TextElement> GetTextLines(List<TextElement> textElements, int windowWidth)
		{
			...
			foreach (TextElement textElement in textElements)
			{
				...
				else if (textElement.text.Length > 0 && textElement.text[0] == '@')
				{
					...
					var tokens = textElement.text.ToString().Substring(1).Split(',');
					int x = int.Parse(tokens[0]);
					int y = int.Parse(tokens[1]);
					var iconSource = new Rectangle(x * Image.ICON_SIZE, y * Image.ICON_SIZE, Image.ICON_SIZE, Image.ICON_SIZE);
					textLines.Add(new TextElement() { line = currentLine, offset = currentLength, color = currentColor, icon = iconSource });
