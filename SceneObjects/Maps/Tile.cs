...
namespace AngelPearl.SceneObjects.Maps
{
    ...
    public class Tile
    {
        ...
        public void Update(GameTime gameTime)
        {
            foreach (TileSprite backgroundSprite in backgroundSprites)
            {
                ...
                {
                    int animFrame = (int)(gameTime.TotalGameTime.TotalMilliseconds / backgroundSprite.animationTime % backgroundSprite.anims.Length);
                    backgroundSprite.source = backgroundSprite.anims[animFrame];
                }
            }
        ...
        public void ApplyTileLayer(TileInstance tile, TilesetDefinition tileset, LayerInstance layer, Rectangle source, Texture2D atlas)
        {
            ...
            if (terrain != null)
            {
				...
				if (Enum.TryParse<TerrainType>(terrain.EnumValueId, out terrainType))
                {
                    Terrain = terrainType;
                    switch (Terrain)
                    {
						case TerrainType.SnowForest:
                        case TerrainType.BarrenForest:
                        case TerrainType.PineForest:
                        case TerrainType.Jungle:
						case TerrainType.Forest:
						case TerrainType.Hills:
                        case TerrainType.DesertHills:
                        case TerrainType.SnowHills:
							blockSight = true;
							break;

						case TerrainType.SnowMountains:
                        case TerrainType.DesertMountains:
                        case TerrainType.GrayMountains:
                        case TerrainType.Volcano:
						case TerrainType.Cairns:
						...
						case TerrainType.Coast:
                            Blocked = true;
                            CanSail = true;
                            BuildAnims(tileSprite, source, [0, 7, 14, 7], 700);
                            break;
        ...
        private void BuildAnims(TileSprite tileSprite, Rectangle source, int[] tokens, int animationTime)
        {
            
            tileSprite.animationTime = animationTime;
            tileSprite.anims = new Rectangle[tokens.Length];
            for (int i = 0; i < tokens.Length; i++)
            {
                tileSprite.anims[i] = new Rectangle(source.X + source.Width * tokens[i], source.Y, source.Width, source.Height);
